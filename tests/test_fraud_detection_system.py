...
class TestFraudDetectionSystem:
    ...
    def test_caso_1_valor_elevado(self):
        ...
        """
        # Parâmetros
        current_transaction = Transaction(amount=15000.00,
                                          timestamp=self.now,
                                          location="Brasil")
    ...
    def test_caso_2_transacoes_excessivas(self):
        ...
        """
        # Parâmetros
        current_transaction = Transaction(amount=500.00,
                                          timestamp=self.now,
                                          location="Brasil")
        # Cria 11 transações nos últimos 59 minutos
        previous_transactions = [
            Transaction(50.0, self.now - timedelta(minutes=i * 5), "Brasil")
            for i in range(1, 12)
        ]
    ...
    def test_score_sum_all_rules_active(self):
        ...
        previous_transactions.append(
            Transaction(15000, self.now - timedelta(minutes=15), "EUA"))
    ...
    def test_limit_time_outside_61_and_30_5_min(self):
        ...
        previous_transactions = [
            ...
            Transaction(100.0, self.now - timedelta(minutes=30, seconds=30),
                        "EUA")
        ]
        # R2: 11 txs @ 61 min
        previous_transactions.extend([
            Transaction(50.0, self.now - timedelta(minutes=61), "Brasil")
            for _ in range(10)
        ])
